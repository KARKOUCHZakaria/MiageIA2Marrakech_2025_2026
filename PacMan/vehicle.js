...
class Vehicle {
  ...
  applyBehaviors(target, obstacles, vehicules) {
    ...
    let boundaryForce = this.avoidBoundaries();
  ...
  */
  ...
  // Comportement Wander : mouvement aléatoire naturel
  wander() {
    // Rayon et angle du cercle de wander
    let wanderRadius = 25;
    let wanderDistance = 60;
    let change = 0.3;

    // Initialiser l'angle de wander s'il n'existe pas
    if (!this.wanderAngle) {
      this.wanderAngle = 0;
    }

    // Changer légèrement l'angle aléatoirement
    this.wanderAngle += random(-change, change);

    // Calculer le point de wander (cercle devant le véhicule)
    let wanderingForce = this.vel.copy();
    wanderingForce.setMag(wanderDistance);
    let target = p5.Vector.add(this.pos, wanderingForce);

    // Ajouter un point sur le cercle
    let wanderPoint = p5.Vector.fromAngle(this.wanderAngle);
    wanderPoint.mult(wanderRadius);
    target.add(wanderPoint);

    // Affichage debug du wander circle
    if (Vehicle.debug) {
      push();
      stroke(100, 255, 100);
      noFill();
      circle(target.x - wanderPoint.x, target.y - wanderPoint.y, wanderRadius * 2);
  ...
  separate(boids) {
    let desiredseparation = Vehicle.separationDistance || (this.r * 2);
    let steer = createVector(0, 0);
    let count = 0;
    // On examine les autres véhicules pour voir s'ils sont trop près
    for (let i = 0; i < boids.length; i++) {
      let other = boids[i];
      if (other === this) continue; // Ignorer soi-même
      
      let d = p5.Vector.dist(this.pos, other.pos);
      // Si la distance est supérieure à 0 et inférieure à la distance désirée
      if (d > 0 && d < desiredseparation) {
        // Vecteur qui pointe loin du voisin
        let diff = p5.Vector.sub(this.pos, other.pos);
        diff.normalize();
        diff.div(d); // Poids en fonction de la distance
        steer.add(diff);
        count++;
      }
    }
  ...
  avoidBoundaries() {
    ...
    if (this.pos.x < d) {
      // Trop à gauche
      steer = createVector(this.maxSpeed, 0);
    } else if (this.pos.x > width - d) {
      // Trop à droite
      steer = createVector(-this.maxSpeed, 0);
    }
    ...
    // Affichage debug des zones dangereuses
    if (Vehicle.debug) {
      push();
      stroke(255, 100, 100);
      strokeWeight(1);
      noFill();
      // Rectangle de sécurité gauche
      rect(0, 0, d, height);
      // Rectangle de sécurité droite
      rect(width - d, 0, d, height);
      // Rectangle de sécurité haut
      rect(0, 0, width, d);
      // Rectangle de sécurité bas
      rect(0, height - d, width, d);
      pop();
    }
